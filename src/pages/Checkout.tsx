  ...
  return (
    <div className="min-h-screen relative">
                          ...
                          {isExpanded && (
                            ...
                            >
                              <div className="pt-2 pb-3 space-y-3">
                                ...
                                {/* Ver fatura (abre modal) */}
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setInvoiceModalSub(subscription);
                                  }}
      ...
      <AnimatePresence>
        ...
        {invoiceModalSub && (
          <Dialog open={!!invoiceModalSub} onOpenChange={(open) => !open && setInvoiceModalSub(null)}>
            <DialogContent className="max-w-md p-0 overflow-hidden">
              {/* Header */}
              <div className="bg-primary px-5 py-4 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-heading font-bold text-primary-foreground">
                    Detalhes da Fatura
                  </h3>
                  <p className="text-xs text-primary-foreground/70">{invoiceModalSub.plan_name}</p>
                </div>
                <Button variant="ghost" size="icon" onClick={() => setInvoiceModalSub(null)} className="text-primary-foreground hover:bg-primary-foreground/10 h-8 w-8">
                  <X className="h-4 w-4" />
                </Button>
              </div>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-base font-heading font-bold text-foreground">{client?.name}</p>
                  {client?.document && (
                    <p className="text-xs text-muted-foreground mt-0.5">CPF/CNPJ: {client.document}</p>
                  )}
                </div>
                ...
                <div className="rounded-xl border border-border/30 overflow-hidden">
                  <div className="bg-primary/10 px-4 py-2">
                    <div className="grid grid-cols-4 gap-2 text-[10px] font-bold text-primary uppercase">
                  ...
                  <div className="px-4 py-3">
                    <div className="grid grid-cols-4 gap-2 text-xs text-foreground">
                      <span className="font-medium">
                        {(() => {
                          const d = new Date(invoiceModalSub.next_payment);
                          const months = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
                          return months[d.getMonth()];
                        })()}
                      </span>
                      <span className="font-mono text-[11px]">{invoiceModalSub.id.split('-')[0].toUpperCase()}</span>
                      <span>{formatBrazilDate(invoiceModalSub.next_payment)}</span>
                      <span className="text-right font-bold">
                        {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(invoiceModalSub.value))}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <Button
                    variant="outline"
                    className="w-full"
                    disabled={generatingInvoice === invoiceModalSub.id}
                    onClick={async () => {
                      if (!client) return;
                      const sub = invoiceModalSub;
                      setGeneratingInvoice(sub.id);
                      toast.info('Gerando fatura em PDF...');
                      try {
                        const pixResult = await createPixPayment({
                          amount: Number(sub.value),
                          description: `Pagamento - ${sub.plan_name}`,
                          clientId: client.id,
                          clientEmail: client.email,
                          clientName: client.name,
                          clientDocument: client.document || undefined,
                        ...
                        if (pixResult?.success && pixResult.qrCode) {
                          generateInvoicePDF({
                            ...
                            dueDate: formatBrazilDate(sub.next_payment),
                  ...
                  >
                    {generatingInvoice === invoiceModalSub.id ? (
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                    ) : (
                      <Download className="h-4 w-4 mr-2" />
                    )}
                    {generatingInvoice === invoiceModalSub.id ? 'Gerando PDF...' : 'Baixar Fatura (PDF)'}
                  </Button>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => {
                      setInvoiceModalSub(null);
                      openPaymentModal(invoiceModalSub);
                    }}
                    className="w-full py-3 px-5 rounded-xl flex items-center justify-center gap-2.5 font-bold text-sm text-white border-none cursor-pointer animate-pulse-pix"
                    style={{ backgroundColor: '#32BCAD', boxShadow: '0 4px 6px rgba(50, 188, 173, 0.3)' }}
                  >
                    <img src={pixIcon} alt="Pix" className="w-5 h-5 invert" />
                    Pagar com Pix
                  </motion.button>
